...
This is the Nagisa netdisk backend, built on the Kratos service template. This
file owns the layering contract agents must follow when changing it.

## Project structure

```
api/<domain>/<version>/   Proto sources and generated stubs. Public contract.
cmd/<app>/                Entrypoint, Wire injector, main.go, config providers.
configs/                  Runtime config (config.yaml). No secrets.
docs/                     Published documentation and the enriched OpenAPI document.
internal/conf/            Config proto; generated by `make config`.
internal/server/          HTTP/gRPC server wiring, middleware, streaming routes.
internal/service/         Transport adapters; one file per resource.
internal/biz/             Domain models, usecases, repo interfaces, errors.
internal/data/            Repo implementations, object storage client, seeding.
internal/pkg/             Leaf utilities with no domain knowledge (crypt, urlsign).
test/integration/         End to end tests; behind the `integration` build tag.
tools/                    Developer commands; `tools/openapi` enriches the document.
web/                      Front end attachment point; `web/dist` is served by the API.
```

`internal/server/middleware` holds the transport concerns: identity extraction,
CORS and the audit trail. `internal/server` also carries the raw binary
bindings (streaming download, zip archive, raw part upload) that cannot be
expressed through the proto JSON codec.

`cmd/<app>/providers.go` owns the providers that turn `conf.Bootstrap` into the
value types `biz` expects. Kept there because wiring is the only place allowed
to see every layer at once.
...
The PowerShell script `scripts/build.ps1` mirrors every Makefile target
(`init`, `api`, `config`, `generate`, `docs`, `all`, `build`, `run`, `test`,
`test-integration`, `fmt`, `vet`, `tidy`, `clean`), so a Windows checkout needs
no GNU make: `.\scripts\build.ps1 all`.

`make all` also runs `make docs`, which regenerates `openapi.yaml` from the
proto sources and then enriches it through `tools/openapi`: document metadata,
security schemes, the anonymous operation list, the shared error responses and
the raw binary bindings. `tools/openapi` refuses to run twice on the same file,
so always regenerate the document before enriching it. The published copies
live in `docs/` (`openapi.yaml`, `swagger.yaml`, `openapi.json`, `index.html`).

Wire's injector is regenerated with `go run github.com/google/wire/cmd/wire@v0.7.0`
from `cmd/<app>`; the version matters because older releases cannot parse a
`go 1.25` module.
